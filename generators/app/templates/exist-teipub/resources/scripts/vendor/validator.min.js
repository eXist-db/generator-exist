...
Number(function (a) {
  'use strict'; function b(b) {
    return this.each(function () {
      var d = a(this); var e = a.extend({}, c.DEFAULTS, d.data(), typeof b === 'object' && b); var f = d.data('bs.validator'); (f || b != 'destroy') && (f || d.data('bs.validator', f = new c(this, e)), typeof b === 'string' && f[b]());
    });
  } var c = function (b, d) {
    this.$element = a(b), this.options = d, d.errors = a.extend({}, c.DEFAULTS.errors, d.errors); for (var e in d.custom) {
      ...
      var b = a(this); var c = b.data('match'); a(c).on('input.bs.validator', function () {
    ...
    var c = a(b.target); var d = c.data('bs.validator.errors'); if (c.is('[type="radio"]') && (c = this.$element.find('input[name="' + c.attr('name') + '"]')), this.$element.trigger(b = a.Event('validate.bs.validator', {relatedTarget: c[0]})), !b.isDefaultPrevented()) {
    ...
    } var e = []; var f = a.Deferred(); var g = this.options; return b.data('bs.validator.deferred') && b.data('bs.validator.deferred').reject(), b.data('bs.validator.deferred', f), a.each(c.VALIDATORS, a.proxy(function (a, c) {
      if ((b.data(a) || a == 'native') && !c.call(this, b)) {
    ...
    var c = this.options.html ? 'html' : 'text'; this.defer(b, function () {
      var d = b.closest('.form-group'); var e = d.find('.help-block.with-errors'); var f = d.find('.form-control-feedback'); var g = b.data('bs.validator.errors'); g.length && (g = a('<ul/>').addClass('list-unstyled').append(a.map(g, function (b) {
        return a('<li/>')[c](b);
    ...
    var b = a.closest('.form-group'); var c = b.find('.help-block.with-errors'); var d = b.find('.form-control-feedback'); c.html(c.data('bs.validator.originalContent')), b.removeClass('has-error'), d.length && d.removeClass(this.options.feedback.error) && d.addClass(this.options.feedback.success) && b.addClass('has-success');
  }, c.prototype.hasErrors = function () {
    function b() {
      return Boolean((a(this).data('bs.validator.errors') || []).length);
    } return Boolean(this.$element.find(c.INPUT_SELECTOR).filter(b).length);
    ...
    } return Boolean(this.$element.find(c.INPUT_SELECTOR).filter('[required]').filter(b).length);
    ...
    return this.$element.removeAttr('novalidate').removeData('bs.validator').off('.bs.validator'), this.$element.find(c.INPUT_SELECTOR).off('.bs.validator').removeData(['bs.validator.errors', 'bs.validator.deferred']).each(function () {
      var b = a(this); var c = b.data('bs.validator.timeout'); window.clearTimeout(c) && b.removeData('bs.validator.timeout');
    }), this.$element.find('.help-block.with-errors').each(function () {
      var b = a(this); var c = b.data('bs.validator.originalContent'); b.removeData('bs.validator.originalContent').html(c);
